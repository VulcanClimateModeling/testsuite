...
def number_of_lines_with_pattern(pattern, input_string, ignore_case=False):
    matching_lines = extract_lines_with_pattern(pattern, input_string, ignore_case)
...
def test_basic_threshold_tuning(number_of_iterations):
    exit_status, stdout, stderr = run_testsuite(
        ['--only=basic,test_basic', '--tune-thresholds', '--tuning-iterations=' + str(number_of_iterations)])
...
def test_updating_of_reference():
    ...
    exit_status, stdout, stderr = run_testsuite(['--only=basic,test_basic', '--update-yufiles'], clean=False)
    check_successful_run(exit_status, stdout, stderr)
...
def test_update_thresholds_argument():
    ...
    # run standard test (and reset thresholds)
    exit_status, stdout, stderr = run_testsuite(['--only=basic,test_basic', '--update-thresholds'], clean=False)
    results = check_test_results(exit_status, stdout, stderr)
